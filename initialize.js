...
;(async () => {
	...
	execSync(
		'npm i --save-dev --legacy-peer-deps @types/lodash babel-plugin-module-resolver @commitlint/config-conventional @commitlint/cli @types/react-native-vector-icons',
		{ stdio: 'inherit' }
	)

	execSync(
		'npm i --save --legacy-peer-deps axios lodash react-native-modal react-native-modal-translucent @react-native-community/async-storage i18next react-i18next react-native-vector-icons @react-navigation/native @react-navigation/stack react-native-reanimated react-native-gesture-handler react-native-screens react-native-safe-area-context @react-native-community/masked-view react-native-fast-image',
		{ stdio: 'inherit' }
	)
	...
	if (select === 'redux') {
		...
		execSync(
			'npm i --save-dev --legacy-peer-deps redux-devtools-extension @types/react-redux',
			{
				stdio: 'inherit',
			}
		)
		execSync(
			'npm i --save --legacy-peer-deps redux redux-saga react-redux redux-persist',
			{
				stdio: 'inherit',
			}
		)
	...
	if (toggle) {
		execSync('npm install --save --legacy-peer-deps @sentry/react-native', {
			stdio: 'inherit',
		})
